...
local intro_win_width = vim.iter(intro):fold(0, function(acc, line) return math.max(acc, #line) end)
intro = vim.iter(intro):map(function(line)
  return string.rep(" ", math.ceil((intro_win_width - #line) / 2)) .. line
end):totable() -- Center all lines
...
local news_win_width = vim.iter(news):fold(0,
  function(acc, line)
...
local features_win_width = vim.iter(features):fold(0,
  function(acc, line)
    return math.max(acc, vim.api.nvim_strwidth(line))
  end
)
...
local header_win_width = vim.iter(header):fold(0,
  function(acc, line)
    return math.max(acc, vim.api.nvim_strwidth(line))
  end
)
